...
#' @param colour_1 for selecting the colour of first player's bars. To be used only for comparison plot.
...
#' @export
#'
#' @examples plot1 = plot_pizza(data = data, type = "comparison", template = "midfielder",
...
data <- fb_player_scouting_report("https://fbref.com/en/players/e5a76dfe/Dean-Henderson", pos_versus = "primary")

plot_pizza <- function(data, type = "", template = "", colour_poss, colour_att, colour_def, player_1, player_2, colour_1, theme = "") {
...
title <- player_name1
subtitle <- player_name2
sub <- data1$Versus
caption <- paste("Compared to", sub, "|", "Last 365 days
Inspired by @FootballSlices")
...
ggplot(data1, aes(x = Statistic, y = Percentile)) +
  geom_bar(aes(y=100),fill= fill_b,stat="identity",width=1,colour= colour_b,                 
           alpha=0.5,show.legend = FALSE) +
  geom_bar(data = data1, aes(fill = colour_1), colour= colour_1, stat="identity", width=1, alpha=1) +
  ...
  theme(plot.background = element_rect(fill = fill_b,color = colour_b),
        ...
        axis.text.x = element_text(size = 12,colour = colorText, angle = ang),
        text = element_text(family="Spartan-Light",colour= colorText, size = 20),                                   
        plot.title = element_markdown(hjust=0.5,family="Spartan-Medium", size = 26, colour = colour_1, face = "bold"),
